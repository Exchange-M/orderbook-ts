...
class Orderbook {
    ...
    add(orderId, side, price, quantity, options) {
        if (this.orderIdMap.has(orderId)) {
            throw new Error(`DUPLICATE ORDERID: ${orderId}`);
        }
        const order = new order_1.default(orderId, side, price, quantity, options);
        ...
        while (oppositeSorted.length > 0 && order.leaveQuantity.gt(ZERO)) {
            ...
            while (bestLevel.head && order.leaveQuantity.gt(ZERO)) {
                ...
                if (order.accountId !== undefined &&
                    ...
                    order.stpMode === order_1.STP_MODE.CANCEL_NEW) {
                ...
                trades.push(new trade_1.default(this.nextTradeId++, sequence, matched.orderId, orderId, oppositeSide, bestLevel.price, matchQty));
                if (fullyFilled) {
